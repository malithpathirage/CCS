    ...
    public class PurchasingController : BaseApiController
    {
        ...
        public async Task<IActionResult> GetSupplier()
        {
            var result = await _context.TransSupplierHeader
                    .Select(x => new { x.SupplierId, x.Name , x.CompanyId, x.ShortCode, x.Address, x.City, x.State, x.ZipPostalCode
                    , x.CountryId, x.Tel, x.Email, x.CurrencyId, x.SupTypeId, x.VATNo, x.TaxNo, x.TinNo, x.CreditDays, x.ShipmentTolorence
                    , x.AccountGroupId, x.bActive, x.LocationId})
                    .ToListAsync();
            return Ok(result);
        }
